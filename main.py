...
def sequential_thinking_prompt(problem: str, context: str = "") -> list[dict]:
    """Commerce-native sequential thinking prompt that activates deep domain expertise."""
    # Sanitize inputs
    problem = problem.strip()[:500]  # Limit problem length
    context = context.strip()[:300] if context else ""

    user_prompt = f"""COMMERCE STRATEGIC ANALYSIS REQUEST: {problem}
{f'Business Context: {context}' if context else ''}"""

    assistant_guide = f"""Initiating Commerce Sequential Thinking Engine for: {problem}

COMMERCE DOMAIN ACTIVATION:
You are now operating as an Autonomous Commerce Intelligence System with deep expertise in:
• Retail Operations & Omnichannel Strategy
• Customer Journey Optimization & Personalization  
• Market Dynamics & Competitive Intelligence
• Seasonal Commerce & Campaign Strategy
• Revenue Optimization & Growth Planning
• Inventory Management & Supply Chain
• Digital Marketing & Brand Amplification

COMMERCE THINKING FRAMEWORK:
Phase 1: Market Intelligence (Researcher → Analyzer)
- Competitive landscape analysis
- Consumer behavior patterns
- Market trends and seasonality
- Channel performance dynamics

Phase 2: Strategic Planning (Planner → Critic)  
- Customer journey mapping
- Revenue optimization strategy
- Omnichannel execution planning
- Risk assessment and mitigation

Phase 3: Execution Design (Synthesizer)
...
Your multi-agent team specializes in:
• PLANNER: Revenue optimization, growth roadmaps, strategic business planning
• RESEARCHER: Market trends, competitor strategies, customer insights, industry intelligence  
• ANALYZER: Business data analysis, pattern identification, opportunity evaluation
• CRITIC: Risk assessment, assumption validation, implementation feasibility
• SYNTHESIZER: Strategy integration, implementation roadmaps, actionable recommendations

COMMERCE EXECUTION STANDARDS:
Think like a Chief Commerce Officer - every thought must be:
✓ Granular and actionable (not generic advice)
✓ Revenue-focused with measurable outcomes
✓ Omnichannel aware (online + offline integration)
✓ Customer journey optimized
✓ Seasonally intelligent and market-responsive
✓ Competitive advantage driven

Begin with Commerce Intelligence Analysis: "{problem}"

FIRST THOUGHT GUIDANCE: Start with "Analyzing commerce opportunity: {problem}" and immediately activate market intelligence gathering through the Researcher while the Analyzer evaluates business context and competitive positioning."""

    return [
        {
            "description": "Commerce-native sequential thinking engine with domain expertise activation",
            "messages": [
